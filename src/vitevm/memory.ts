...
// taken from https://github.com/ethereumjs/ethereumjs-monorepo/blob/master/packages/evm/src/memory.ts
...
export class Memory {
    ...
    extend(offset: number, size: number) {
        ...
        const sizeDiff = newSize - this.store.length
        if (sizeDiff > 0) {
            this.store = Buffer.concat([this.store, Buffer.alloc(sizeDiff)])
        }
    }
    ...
    write(offset: number, size: number, value: Buffer) {
        ...
        if (offset + size > this.store.length) throw new Error("Value exceeds memory capacity")
    ...
    read(offset: number, size: number): Buffer {
        ...
        const loaded = Buffer.from(this.store.slice(offset, offset + size))
